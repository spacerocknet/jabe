...
object TestDeviceDAO {
  def main(args: Array[String]): Unit = {
    ...
    running(app) {
      ...
      device.addNewDevice(d1)
      device.addNewDevice(d2)
      device.addNewDevice(d3)
      device.addNewDevice(d5)

      device.addNewDevice("device 6", new Date(), "uid 6", "os 6", "platform 6",
        "model 6", "phone 6")
